...
typedef struct {
    ngx_buf_t               *in_buf;
    ngx_buf_t               *out_buf;
    ngx_http_udbroker_range_t  current_range_n;
    off_t                   *parser_state;
    ngx_log_t               *log;
...
} ngx_http_udbroker_merger_state_t;
...
} ngx_http_udbroker_field_template_t;
...
typedef struct {
    ...
    ngx_str_t                text;
#endif
} ngx_http_udbroker_field_filter_t;

/*
 * Upload cleanup record
 */
typedef struct ngx_http_udbroker_cleanup_s {
    ngx_fd_t                         fd;
    u_char                           *filename;
...
} ngx_http_udbroker_loc_conf_t;

typedef struct ngx_http_udbroker_md5_ctx_s {
    MD5_CTX     md5;
    u_char      md5_digest[MD5_DIGEST_LENGTH * 2];
} ngx_http_udbroker_md5_ctx_t;

typedef struct ngx_http_udbroker_sha1_ctx_s {
    SHA_CTX     sha1;
    u_char      sha1_digest[SHA_DIGEST_LENGTH * 2];
} ngx_http_udbroker_sha1_ctx_t;

struct ngx_http_udbroker_ctx_s;

/*
 * Request body data handler
 */
typedef ngx_int_t (*ngx_http_request_body_data_handler_pt)
    (struct ngx_http_udbroker_ctx_s*, u_char *, u_char*);

/*
 * Upload module context
 */
typedef struct ngx_http_udbroker_ctx_s {
    ...
    ngx_http_udbroker_range_t     content_range_n;
    ...
    ngx_int_t (*start_part_f)(struct ngx_http_udbroker_ctx_s *upload_ctx);
    void (*finish_part_f)(struct ngx_http_udbroker_ctx_s *upload_ctx);
    void (*abort_part_f)(struct ngx_http_udbroker_ctx_s *upload_ctx);
	ngx_int_t (*flush_output_buffer_f)(struct ngx_http_udbroker_ctx_s *upload_ctx, u_char *buf, size_t len);
    ...
    ngx_http_udbroker_md5_ctx_t   *md5_ctx;    
    ngx_http_udbroker_sha1_ctx_t  *sha1_ctx;    
    uint32_t                    crc32;    
...
} ngx_http_udbroker_ctx_t;

static ngx_int_t ngx_http_udbroker_handler(ngx_http_request_t *r);
static ngx_int_t ngx_http_udbroker_body_handler(ngx_http_request_t *r);

static void *ngx_http_udbroker_create_loc_conf(ngx_conf_t *cf);
static char *ngx_http_udbroker_merge_loc_conf(ngx_conf_t *cf,
    void *parent, void *child);
static ngx_int_t ngx_http_udbroker_add_variables(ngx_conf_t *cf);
static void ngx_http_udbroker_variable_set(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_http_udbroker_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_http_udbroker_md5_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_http_udbroker_sha1_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_http_udbroker_file_size_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
static void ngx_http_udbroker_content_range_variable_set(ngx_http_request_t *r,
    ngx_http_variable_value_t *v,  uintptr_t data);
static ngx_int_t ngx_http_udbroker_content_range_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v,  uintptr_t data);
static ngx_int_t ngx_http_udbroker_crc32_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_http_udbroker_uint_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
static char *ngx_http_udbroker_pass(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);

static ngx_int_t ngx_http_udbroker_start_handler(ngx_http_udbroker_ctx_t *u);
static void ngx_http_udbroker_finish_handler(ngx_http_udbroker_ctx_t *u);
static void ngx_http_udbroker_abort_handler(ngx_http_udbroker_ctx_t *u);

static ngx_int_t ngx_http_udbroker_flush_output_buffer(ngx_http_udbroker_ctx_t *u,
    u_char *buf, size_t len);
static ngx_int_t ngx_http_udbroker_append_field(ngx_http_udbroker_ctx_t *u,
    ngx_str_t *name, ngx_str_t *value);
static ngx_int_t ngx_http_udbroker_merge_ranges(ngx_http_udbroker_ctx_t *u, ngx_http_udbroker_range_t *range_n);
static ngx_int_t ngx_http_udbroker_parse_range(ngx_str_t *range, ngx_http_udbroker_range_t *range_n);
...
static char *ngx_http_udbroker_set_form_field(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
static char *ngx_http_udbroker_pass_form_field(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
static char *ngx_http_udbroker_cleanup(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
static void ngx_upload_cleanup_handler(void *data);

#if defined nginx_version && nginx_version >= 7052
static ngx_path_init_t        ngx_http_udbroker_temp_path = {
 ...
 * 
 */
static void upload_init_ctx(ngx_http_udbroker_ctx_t *upload_ctx);
 ...
 * 
 */
static void upload_shutdown_ctx(ngx_http_udbroker_ctx_t *upload_ctx);
 ...
 *
 */
static ngx_int_t upload_start(ngx_http_udbroker_ctx_t *upload_ctx, ngx_http_udbroker_loc_conf_t  *ulcf);
 ...
 *     NGX_ERROR if error has occured
 */
static ngx_int_t upload_parse_request_headers(ngx_http_udbroker_ctx_t *upload_ctx, ngx_http_headers_in_t *headers_in);
...
static char * ngx_http_udbroker(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);

static ngx_int_t upload_process_buf(ngx_http_udbroker_ctx_t *upload_ctx, u_char *start, u_char *end);
static ngx_int_t upload_process_raw_buf(ngx_http_udbroker_ctx_t *upload_ctx, u_char *start, u_char *end);

static ngx_command_t  ngx_http_udbroker_commands[] = { /* {{{ */
    ...
    { ngx_string("upload_store"),
      ...
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_udbroker_loc_conf_t, store_path),
      NULL },
    ...
    { ngx_string("upload_state_store"),
      ...
      ngx_conf_set_path_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_udbroker_loc_conf_t, state_store_path),
      NULL },
    ...
    { ngx_string("upload_store_access"),
      ...
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_udbroker_loc_conf_t, store_access),
      NULL },
    ...
    { ngx_string("upload_buffer_size"),
      ...
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_udbroker_loc_conf_t, buffer_size),
      NULL },
    ...
    { ngx_string("upload_merge_buffer_size"),
      ...
      ngx_conf_set_size_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_udbroker_loc_conf_t, merge_buffer_size),
      NULL },
    ...
    { ngx_string("upload_range_header_buffer_size"),
      ...
      ngx_conf_set_size_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_udbroker_loc_conf_t, range_header_buffer_size),
      NULL },
    ...
    { ngx_string("upload_max_part_header_len"),
      ...
      ngx_conf_set_size_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_udbroker_loc_conf_t, max_header_len),
      NULL },
    ...
    { ngx_string("upload_max_file_size"),
      ...
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_udbroker_loc_conf_t, max_file_size),
      NULL },
    ...
    { ngx_string("upload_max_output_body_len"),
      ...
      ngx_conf_set_size_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_udbroker_loc_conf_t, max_output_body_len),
      NULL },
    ...
    { ngx_string("upload_set_form_field"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_HTTP_LMT_CONF|NGX_HTTP_LIF_CONF
                        |NGX_CONF_TAKE2,
      ngx_http_udbroker_set_form_field,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_udbroker_loc_conf_t, field_templates),
      NULL},
    ...
    { ngx_string("upload_aggregate_form_field"),
      ...
      ngx_http_udbroker_set_form_field,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_udbroker_loc_conf_t, aggregate_field_templates),
      NULL},
    ...
    { ngx_string("upload_pass_form_field"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_HTTP_LMT_CONF|NGX_HTTP_LIF_CONF
                        |NGX_CONF_TAKE1,
      ngx_http_udbroker_pass_form_field,
      NGX_HTTP_LOC_CONF_OFFSET,
      0,
      NULL},
    ...
    { ngx_string("upload_cleanup"),
     ...
     { ngx_string("upload_pass_args"),
       ...
       NGX_HTTP_LOC_CONF_OFFSET,
       offsetof(ngx_http_udbroker_loc_conf_t, forward_args),
       NULL },
    ...
    { ngx_string("upload_limit_rate"),
      ...
      ngx_conf_set_size_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_udbroker_loc_conf_t, limit_rate),
      NULL },
     ...
     { ngx_string("upload_tame_arrays"),
       ...
       ngx_conf_set_flag_slot,
       NGX_HTTP_LOC_CONF_OFFSET,
       offsetof(ngx_http_udbroker_loc_conf_t, tame_arrays),
       NULL },
     ...
     { ngx_string("upload_resumable"),
       ...
       ngx_conf_set_flag_slot,
       NGX_HTTP_LOC_CONF_OFFSET,
       offsetof(ngx_http_udbroker_loc_conf_t, resumable_uploads),
       NULL },

      ngx_null_command
}; /* }}} */

ngx_http_module_t  ngx_http_udbroker_module_ctx = { /* {{{ */
    ngx_http_udbroker_add_variables,         /* preconfiguration */
    ...
    ngx_http_udbroker_create_loc_conf,       /* create location configuration */
    ngx_http_udbroker_merge_loc_conf         /* merge location configuration */
}; /* }}} */

ngx_module_t  ngx_http_udbroker_module = { /* {{{ */
    NGX_MODULE_V1,
    &ngx_http_udbroker_module_ctx,           /* module context */
    ngx_http_udbroker_commands,              /* module directives */
...
static ngx_http_variable_t  ngx_http_udbroker_variables[] = { /* {{{ */

    { ngx_string("upload_field_name"), NULL, ngx_http_udbroker_variable,
      (uintptr_t) offsetof(ngx_http_udbroker_ctx_t, field_name),
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_string("upload_content_type"),
      ngx_http_udbroker_variable_set,
      ngx_http_udbroker_variable,
      (uintptr_t) offsetof(ngx_http_udbroker_ctx_t, content_type),
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_string("upload_file_name"), NULL, ngx_http_udbroker_variable,
      (uintptr_t) offsetof(ngx_http_udbroker_ctx_t, file_name),
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_string("upload_file_number"), NULL, ngx_http_udbroker_uint_variable,
      (uintptr_t) offsetof(ngx_http_udbroker_ctx_t, ordinal),
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_string("upload_tmp_path"), NULL, ngx_http_udbroker_variable,
      (uintptr_t) offsetof(ngx_http_udbroker_ctx_t, output_file.name),
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_string("upload_content_range"),
      ngx_http_udbroker_content_range_variable_set,
      ngx_http_udbroker_content_range_variable,
      (uintptr_t) offsetof(ngx_http_udbroker_ctx_t, content_range_n),
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_null_string, NULL, NULL, 0, 0, 0 }
}; /* }}} */

static ngx_http_variable_t  ngx_http_udbroker_aggregate_variables[] = { /* {{{ */

    { ngx_string("upload_file_md5"), NULL, ngx_http_udbroker_md5_variable,
      (uintptr_t) "0123456789abcdef",
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_string("upload_file_md5_uc"), NULL, ngx_http_udbroker_md5_variable,
      (uintptr_t) "0123456789ABCDEF",
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_string("upload_file_sha1"), NULL, ngx_http_udbroker_sha1_variable,
      (uintptr_t) "0123456789abcdef",
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_string("upload_file_sha1_uc"), NULL, ngx_http_udbroker_sha1_variable,
      (uintptr_t) "0123456789ABCDEF",
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_string("upload_file_crc32"), NULL, ngx_http_udbroker_crc32_variable,
      (uintptr_t) offsetof(ngx_http_udbroker_ctx_t, crc32),
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_string("upload_file_size"), NULL, ngx_http_udbroker_file_size_variable,
      (uintptr_t) offsetof(ngx_http_udbroker_ctx_t, output_file.offset),
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },

    { ngx_null_string, NULL, NULL, 0, 0, 0 }
}; /* }}} */

static ngx_str_t  ngx_http_udbroker_empty_field_value = ngx_null_string;
...
static u_char  demo_html[] = "<html><body>udbroker OK.</body></html>";

ngx_int_t response_to_client(ngx_http_request_t *r)
{
    ngx_int_t                 rc;
    ...
    b->pos = b->start;
    b->last = b->start;
    b->end = b->last + sizeof(demo_html) - 1;
    b->memory = 1;
...
static ngx_int_t /* {{{ ngx_http_udbroker_handler */
ngx_http_udbroker_handler(ngx_http_request_t *r)
{
    ngx_http_udbroker_loc_conf_t  *ulcf;
    ngx_http_udbroker_ctx_t     *u;
    ngx_int_t                 rc;

    if (!(r->method & NGX_HTTP_POST))
        return NGX_HTTP_NOT_ALLOWED;

    ulcf = ngx_http_get_module_loc_conf(r, ngx_http_udbroker_module);

    u = ngx_http_get_module_ctx(r, ngx_http_udbroker_module);

    if (u == NULL) {
        u = ngx_pcalloc(r->pool, sizeof(ngx_http_udbroker_ctx_t));
        if (u == NULL) {
            return NGX_HTTP_INTERNAL_SERVER_ERROR;
        }

        ngx_http_set_ctx(r, u, ngx_http_udbroker_module);
    }

    if(ulcf->md5) {
        if(u->md5_ctx == NULL) {
            u->md5_ctx = ngx_palloc(r->pool, sizeof(ngx_http_udbroker_md5_ctx_t));
            if (u->md5_ctx == NULL) {
                return NGX_HTTP_INTERNAL_SERVER_ERROR;
            }
        }
    ...
    if(ulcf->sha1) {
        if(u->sha1_ctx == NULL) {
            u->sha1_ctx = ngx_palloc(r->pool, sizeof(ngx_http_udbroker_sha1_ctx_t));
            if (u->sha1_ctx == NULL) {
                return NGX_HTTP_INTERNAL_SERVER_ERROR;
            }
        }
...
static ngx_int_t ngx_http_udbroker_body_handler(ngx_http_request_t *r) { /* {{{ */
    ngx_http_udbroker_loc_conf_t  *ulcf = ngx_http_get_module_loc_conf(r, ngx_http_udbroker_module);
    ngx_http_udbroker_ctx_t       *ctx = ngx_http_get_module_ctx(r, ngx_http_udbroker_module);
    ...
    if(ctx->no_content) {
        rc = ngx_http_udbroker_append_field(ctx, &dummy, &ngx_http_udbroker_empty_field_value);
...
static ngx_int_t ngx_http_udbroker_start_handler(ngx_http_udbroker_ctx_t *u) { /* {{{ */
    ...
    ngx_int_t   rc;
    /*ngx_err_t   err;*/
    ngx_http_udbroker_field_template_t    *t;
    ngx_http_udbroker_field_filter_t    *f;
    ...
    if(u->is_file) {
        ...
        if(ulcf->field_templates) {
            ...
            for (i = 0; i < ulcf->field_templates->nelts; i++) {
                ...
                rc = ngx_http_udbroker_append_field(u, &field_name, &field_value);
        ...
        if(pass_field && u->field_name.len > 0) { 
             ...
             * is not known until ngx_http_udbroker_flush_output_buffer
             * is called. We pass empty field value to simplify things.
             */
            rc = ngx_http_udbroker_append_field(u, &u->field_name, &ngx_http_udbroker_empty_field_value);
...
static void ngx_http_udbroker_finish_handler(ngx_http_udbroker_ctx_t *u) { /* {{{ */
    ngx_http_udbroker_field_template_t    *af;
    ...
    if(u->is_file) {
        ...
        if(u->partial_content) {
            ...
            rc = ngx_http_udbroker_merge_ranges(u, &u->content_range_n);
        ...
        if(ulcf->aggregate_field_templates) {
            ...
            for (i = 0; i < ulcf->aggregate_field_templates->nelts; i++) {
                ...
                rc = ngx_http_udbroker_append_field(u, &aggregate_field_name, &aggregate_field_value);
...
rollback:
    ngx_http_udbroker_abort_handler(u);
} /* }}} */
...
static ngx_int_t ngx_http_udbroker_flush_output_buffer(ngx_http_udbroker_ctx_t *u, u_char *buf, size_t len) { /* {{{ */
    ngx_http_request_t             *r = u->request;
    ngx_buf_t                      *b;
    ngx_chain_t                    *cl;
    ngx_http_udbroker_loc_conf_t     *ulcf = ngx_http_get_module_loc_conf(r, ngx_http_udbroker_module);
...
static void /* {{{ ngx_http_udbroker_append_str */
ngx_http_udbroker_append_str(ngx_http_udbroker_ctx_t *u, ngx_buf_t *b, ngx_chain_t *cl, ngx_str_t *s)
{
...
static ngx_int_t /* {{{ ngx_http_udbroker_append_field */
ngx_http_udbroker_append_field(ngx_http_udbroker_ctx_t *u, ngx_str_t *name, ngx_str_t *value)
{
    ngx_http_udbroker_loc_conf_t     *ulcf = ngx_http_get_module_loc_conf(u->request, ngx_http_udbroker_module);
    ...
    if(name->len > 0) {
        ...
        ngx_http_udbroker_append_str(u, b, cl, &boundary);

        ngx_http_udbroker_append_str(u, b + 1, cl + 1, &ngx_upload_field_part1);

        ngx_http_udbroker_append_str(u, b + 2, cl + 2, name);

        ngx_http_udbroker_append_str(u, b + 3, cl + 3, &ngx_upload_field_part2);

        if(value->len > 0)
            ngx_http_udbroker_append_str(u, b + 4, cl + 4, value);
...
static ngx_int_t ngx_http_udbroker_add_range(ngx_http_udbroker_merger_state_t *ms, ngx_http_udbroker_range_t *range_n) {
...
static ngx_int_t /* {{{ ngx_http_udbroker_buf_merge_range */
ngx_http_udbroker_buf_merge_range(ngx_http_udbroker_merger_state_t *ms, ngx_http_udbroker_range_t *range_n) {
    u_char *p, c;
    off_t                  *field;
        ...
        while(p != ms->in_buf->last) {
            ...
            if(c >= '0' && c <= '9') {
                ...
                if(ms->current_range_n.end + 1 < range_n->start) {
                     ...
                     */
                    if(ngx_http_udbroker_add_range(ms, &ms->current_range_n) != NGX_OK) {
                        return NGX_ERROR;
                    }
                ...
                if(ms->current_range_n.start > range_n->end + 1) {
                    ...
                    if(!ms->found_lower_bound) {
                        if(ngx_http_udbroker_add_range(ms, range_n) != NGX_OK) {
                            return NGX_ERROR;
                        }
                    }
...
static ngx_int_t /* {{{ ngx_http_udbroker_merge_ranges */
ngx_http_udbroker_merge_ranges(ngx_http_udbroker_ctx_t *u, ngx_http_udbroker_range_t *range_n) {
    ngx_file_t  *state_file = &u->state_file;
    ngx_http_udbroker_merger_state_t ms;
    off_t        remaining;
    ssize_t      rc;
    int          result;
    ngx_buf_t    in_buf;
    ngx_buf_t    out_buf;
    ngx_http_udbroker_loc_conf_t  *ulcf = ngx_http_get_module_loc_conf(u->request, ngx_http_udbroker_module);
    ngx_http_udbroker_range_t  range_to_merge_n;
    ...
    do {
        ...
        if(ngx_http_udbroker_buf_merge_range(&ms, &range_to_merge_n) != NGX_OK) {
    ...
    if(out_buf.file_pos < state_file->info.st_size) {
        ...
        if ( result != 0 ){
            ngx_log_error(NGX_LOG_ERR, u->request->connection->log, 0,
                    "ftruncate() failed. result=%d", result);
        }
    }
...
static void * /* {{{ ngx_http_udbroker_create_loc_conf */
ngx_http_udbroker_create_loc_conf(ngx_conf_t *cf)
{
    ngx_http_udbroker_loc_conf_t  *conf;

    conf = ngx_pcalloc(cf->pool, sizeof(ngx_http_udbroker_loc_conf_t));
    if (conf == NULL) {
...
static char * /* {{{ ngx_http_udbroker_merge_loc_conf */
ngx_http_udbroker_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ngx_http_udbroker_loc_conf_t  *prev = parent;
    ngx_http_udbroker_loc_conf_t  *conf = child;
...
static ngx_int_t /* {{{ ngx_http_udbroker_add_variables */
ngx_http_udbroker_add_variables(ngx_conf_t *cf)
{
    ngx_http_variable_t  *var, *v;

    for (v = ngx_http_udbroker_variables; v->name.len; v++) {
    ...
    for (v = ngx_http_udbroker_aggregate_variables; v->name.len; v++) {
        var = ngx_http_add_variable(cf, &v->name, v->flags);
        if (var == NULL) {
            return NGX_ERROR;
        }
...
static void /* {{{ ngx_http_udbroker_variable_set */
ngx_http_udbroker_variable_set(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data)
{
    ngx_str_t  *s;
    ngx_http_udbroker_ctx_t  *u;
...
static ngx_int_t /* {{{ ngx_http_udbroker_variable */
ngx_http_udbroker_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data)
{
    ngx_http_udbroker_ctx_t  *u;
    ngx_str_t              *value;
...
static ngx_int_t /* {{{ ngx_http_udbroker_md5_variable */
ngx_http_udbroker_md5_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v,  uintptr_t data)
{
    ngx_uint_t             i;
    ngx_http_udbroker_ctx_t  *u;
...
static ngx_int_t /* {{{ ngx_http_udbroker_sha1_variable */
ngx_http_udbroker_sha1_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v,  uintptr_t data)
{
    ngx_uint_t             i;
    ngx_http_udbroker_ctx_t  *u;
...
static ngx_int_t /* {{{ ngx_http_udbroker_crc32_variable */
ngx_http_udbroker_crc32_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v,  uintptr_t data)
{
    ngx_http_udbroker_ctx_t  *u;
    u_char                 *p;
...
static ngx_int_t /* {{{ ngx_http_udbroker_file_size_variable */
ngx_http_udbroker_file_size_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v,  uintptr_t data)
{
    ngx_http_udbroker_ctx_t  *u;
    u_char                 *p;
...
static void /* {{{ ngx_http_udbroker_content_range_variable_set */
ngx_http_udbroker_content_range_variable_set(ngx_http_request_t *r,
    ngx_http_variable_value_t *v,  uintptr_t data)
{
    ngx_http_udbroker_ctx_t   *u;
    ngx_str_t                val;
    ngx_http_udbroker_range_t *value;

    u = ngx_http_get_module_ctx(r, ngx_http_udbroker_module);

    value = (ngx_http_udbroker_range_t *) ((char *) u + data);

    val.len = v->len;
    val.data = v->data;

    if(ngx_http_udbroker_parse_range(&val, value) != NGX_OK) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "invalid range \"%V\"", &val);
    }
} /* }}} */

static ngx_int_t /* {{{ ngx_http_udbroker_content_range_variable */
ngx_http_udbroker_content_range_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v,  uintptr_t data)
{
    ngx_http_udbroker_ctx_t  *u;
    u_char                 *p;
...
static ngx_int_t /* {{{ ngx_http_udbroker_uint_variable */
ngx_http_udbroker_uint_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v,  uintptr_t data)
{
    ngx_http_udbroker_ctx_t  *u;
    u_char                 *p;
...
static char * /* {{{ ngx_http_udbroker_set_form_field */
ngx_http_udbroker_set_form_field(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_int_t                   n, i;
    ngx_str_t                  *value;
    ngx_http_script_compile_t   sc;
    ngx_http_udbroker_field_template_t *h;
    ngx_array_t                 **field;
    ngx_http_variable_t         *v;
    u_char                      *match;
    ngx_http_udbroker_loc_conf_t  *ulcf = conf;
    ...
    if (*field == NULL) {
        *field = ngx_array_create(cf->pool, 1,
                                  sizeof(ngx_http_udbroker_field_template_t));
        if (*field == NULL) {
            return NGX_CONF_ERROR;
        }
    }
    ...
    for(i = 1;i <= 2;i++) {
        for (v = ngx_http_udbroker_aggregate_variables; v->name.len; v++) {
            ...
            {
                if(cmd->offset != offsetof(ngx_http_udbroker_loc_conf_t, aggregate_field_templates)) {
                ...
                if(v->get_handler == ngx_http_udbroker_md5_variable)
                    ulcf->md5 = 1;

                if(v->get_handler == ngx_http_udbroker_sha1_variable)
                    ulcf->sha1 = 1;

                if(v->get_handler == ngx_http_udbroker_crc32_variable)
                    ulcf->crc32 = 1;
            }
        }
    }
...
static char * /* {{{ ngx_http_udbroker_pass_form_field */
ngx_http_udbroker_pass_form_field(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_http_udbroker_loc_conf_t *ulcf = conf;
    ...
    if (ulcf->field_filters == NULL) {
        ulcf->field_filters = ngx_array_create(cf->pool, 1,
                                        sizeof(ngx_http_udbroker_field_filter_t));
        if (ulcf->field_filters == NULL) {
            return NGX_CONF_ERROR;
        }
    }
...
static char * /* {{{ ngx_http_udbroker_cleanup */
ngx_http_udbroker_cleanup(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_http_udbroker_loc_conf_t *ulcf = conf;
...
static char *
ngx_http_udbroker(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_http_core_loc_conf_t  *clcf;

    clcf = ngx_http_conf_get_module_loc_conf(cf, ngx_http_core_module);
    clcf->handler = ngx_http_udbroker_handler;

    return NGX_CONF_OK;
}

static char * /* {{{ ngx_http_udbroker_pass */
ngx_http_udbroker_pass(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_http_core_loc_conf_t          *clcf;
    ngx_http_udbroker_loc_conf_t        *ulcf = conf;
    ngx_str_t                         *value;
...
ngx_http_read_upload_client_request_body(ngx_http_request_t *r) {
    ...
    ngx_http_core_loc_conf_t  *clcf;
    ngx_http_udbroker_ctx_t     *u = ngx_http_get_module_ctx(r, ngx_http_udbroker_module);
    ...
    if (preread) {
        ...
        if (preread >= r->headers_in.content_length_n) {
            ...
            return ngx_http_udbroker_body_handler(r);
        }
...
static ngx_int_t upload_parse_content_disposition(ngx_http_udbroker_ctx_t *upload_ctx, ngx_str_t *content_disposition) { /* {{{ */
...
static ngx_int_t upload_parse_part_header(ngx_http_udbroker_ctx_t *upload_ctx, char *header, char *header_end) { /* {{{ */
    ngx_str_t s;
...
static void upload_discard_part_attributes(ngx_http_udbroker_ctx_t *upload_ctx) { /* {{{ */
...
static ngx_int_t upload_start_file(ngx_http_udbroker_ctx_t *upload_ctx) { /* {{{ */
...
static void upload_finish_file(ngx_http_udbroker_ctx_t *upload_ctx) { /* {{{ */
...
static void upload_abort_file(ngx_http_udbroker_ctx_t *upload_ctx) { /* {{{ */
...
static void upload_flush_output_buffer(ngx_http_udbroker_ctx_t *upload_ctx) { /* {{{ */
...
static void upload_init_ctx(ngx_http_udbroker_ctx_t *upload_ctx) { /* {{{ */
	...
	upload_ctx->start_part_f = ngx_http_udbroker_start_handler;
	upload_ctx->finish_part_f = ngx_http_udbroker_finish_handler;
	upload_ctx->abort_part_f = ngx_http_udbroker_abort_handler;
	upload_ctx->flush_output_buffer_f = ngx_http_udbroker_flush_output_buffer;
...
static ngx_int_t /* {{{ ngx_http_udbroker_validate_session_id */
ngx_http_udbroker_validate_session_id(ngx_str_t *session_id) {
    u_char *p, *q;
...
static ngx_int_t upload_parse_request_headers(ngx_http_udbroker_ctx_t *upload_ctx, ngx_http_headers_in_t *headers_in) { /* {{{ */
    ...
    ulcf = ngx_http_get_module_loc_conf(upload_ctx->request, ngx_http_udbroker_module);
    ...
    if(ngx_strncasecmp(content_type->data, (u_char*) MULTIPART_FORM_DATA_STRING,
        ...
        for (i = 0;;i++) {
                ...
                || !strncasecmp(X_SESSION_ID_STRING, (char*)header[i].key.data, sizeof(X_SESSION_ID_STRING) - 1 - 1))
            {
                ...
                if(ngx_http_udbroker_validate_session_id(&header[i].value) != NGX_OK) {
                    ngx_log_debug0(NGX_LOG_DEBUG_CORE, upload_ctx->log, 0,
                ...
                || !strncasecmp(X_CONTENT_RANGE_STRING, (char*)header[i].key.data, sizeof(X_CONTENT_RANGE_STRING) - 1 - 1))
            {
                ...
                if(ngx_http_udbroker_parse_range(&s, &upload_ctx->content_range_n) != NGX_OK) {
...
static ngx_int_t /* {{{ ngx_http_udbroker_parse_range */
ngx_http_udbroker_parse_range(ngx_str_t *range, ngx_http_udbroker_range_t *range_n)
{
    u_char *p = range->data;
...
static void upload_putc(ngx_http_udbroker_ctx_t *upload_ctx, u_char c) { /* {{{ */
...
static ngx_int_t
upload_process_raw_buf(ngx_http_udbroker_ctx_t *upload_ctx, u_char *start, u_char *end) { /* {{{ */
    ngx_int_t rc;
