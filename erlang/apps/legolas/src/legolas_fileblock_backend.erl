...
start(Partition, Config) ->
    ...
    BPrefixList = case common_utils:get_prop_or_env(
...
open_db(#state{ref=Ref} = State) ->
    case Ref of
        undefined ->
            RetriesLeft = common_utils:get_env(legolas, eleveldb_open_retries, 3),
    ...
    case eleveldb:open(State0#state.indexes_datadir, State0#state.open_opts) of
        ...
        {error, {db_open, OpenErr}=Reason} ->
            case lists:prefix("IO error: lock ", OpenErr) of
                true ->
                    SleepFor = common_utils:get_env(legolas, eleveldb_open_retry_delay, 2000),
