 ...
 */
struct slist_head* genc_slist_find_entry(struct slist_head* start, genc_slist_entry_pred_fn pred, void* data) __attribute__ ((no_instrument_function));
 ...
 * pointer.
 */
struct slist_head** genc_slist_find_entry_ref(struct slist_head** start, genc_slist_entry_pred_fn pred, void* data) __attribute__ ((no_instrument_function));
 ...
 */
struct slist_head** genc_slist_insert_at(struct slist_head* new_entry, struct slist_head** at) __attribute__ ((no_instrument_function));

/** Inserts a single new list element after an existing entry.
 */
void genc_slist_insert_after(struct slist_head* new_entry, struct slist_head* after_entry) __attribute__ ((no_instrument_function));
 ...
 */
struct slist_head* genc_slist_remove_at(struct slist_head** at) __attribute__ ((no_instrument_function));

/** Removes the element following the given list element and returns it, or NULL if there was no such element.
 */
struct slist_head* genc_slist_remove_after(struct slist_head* after_entry) __attribute__ ((no_instrument_function));

struct slist_head** genc_slist_find_tail(struct slist_head** head) __attribute__ ((no_instrument_function));
...
struct slist_head** genc_slist_splice(struct slist_head** into, struct slist_head** from) __attribute__ ((no_instrument_function));

/* Iterates the list until the end is reached, and returns the number of
 * elements encountered. Runtime is therefore O(N). */
size_t genc_slist_length(struct slist_head* list) __attribute__ ((no_instrument_function));
 ...
 */
struct genc_slist_ref_pair genc_slist_find_common_tail_refs(struct slist_head** list_a, struct slist_head** list_b) __attribute__ ((no_instrument_function));
...
struct slist_head* genc_slist_find_common_tail(struct slist_head* list_a, struct slist_head* list_b) __attribute__ ((no_instrument_function));

genc_bool_t genc_slist_is_empty(struct slist_head* list) __attribute__ ((no_instrument_function));

genc_slist_head_t** genc_slist_find_ref(genc_slist_head_t* item, genc_slist_head_t** list) __attribute__ ((no_instrument_function));
 ...
 * };
 *
 * Uses the genc_container_of() macro from genccont_util.h
 */
